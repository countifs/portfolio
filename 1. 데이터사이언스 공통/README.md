...
- gui의 `복사` 버튼을 누르면 결과가 클립보드로 전달되며, `조회하기` 또는 `clear`버튼을 누르면 이전 결과는 리셋이 됩니다.
- `구인공고 바로가기` 버튼을 누르면 해당 구인번호의 채용공고가 있는 웹페이지 화면으로 이동합니다.
- 해당 프로그램은 python으로 작성 후, exe파일로 제작되어 배포되었습니다.
